...
}this.viewer.setTrajectoryBs (J.util.BSUtil.newBitSet2 (baseModelIndex, this.viewer.modelSet.modelCount));
...
this.pymolScene =  new J.adapter.readers.pymol.PyMOLScene (this, this.viewer, settings, this.uniqueSettings, this.pymolVersion, this.haveScenes);
...
case 3:
case 2:
this.processMap (pymolObject, type == 3);
break;
case 8:
...
$_M(c$, "processGadget", 
($fz = function (pymolObject) {
if (this.objectName.endsWith ("_e_pot")) this.processMap (pymolObject, true);
}, $fz.isPrivate = true, $fz), "J.util.JmolList");
$_M(c$, "processMap", 
($fz = function (pymolObject, isObject) {
if (isObject) {
if (this.isStateScript) return;
if (this.isHidden) return;
...
var atom = this.processAtom ( new J.adapter.smarter.Atom (), name, altLoc.charAt (0), group3, chainID.charAt (0), seqNo, insCode.charAt (0), isHetero, sym);
...
if (lastAtomChain != (this.atoms[i].chainID).charCodeAt (0) || lastAtomSet != this.atoms[i].atomSetIndex) {
newChain[i] = true;
lastAtomChain = (this.atoms[i].chainID).charCodeAt (0);
